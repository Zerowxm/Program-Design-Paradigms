...
public class Test{
	public static void main(String[] args) {
        ...
        checkEqual(Rosters.empty().without(p),Rosters.empty(),"Test without()");
        checkEqual(r.without(p).without(p),r.without(p),"Test without()");

        checkTrue(!Rosters.empty().has(p),"Test has()");

        checkTrue(Rosters.empty().with(p).has(p),"Test has()");

        checkTrue(!Rosters.empty().with(p).without(p).has(p),"Test has()");

        checkEqual(Rosters.empty().size(),0,"Test size()");
        checkEqual(Rosters.empty().without(p).size(),0,"Test size()");
        checkEqual(Rosters.empty().with(p).without(p).size(),0,"Test size()");
        checkEqual(Rosters.empty().with(p).size(),1,"Test size()");
        checkEqual(Rosters.empty().with(p).with(p).size(),1,"Test size()");

        Roster allReady = Rosters.empty();
        ...
        Roster someReady = Rosters.empty();
        ...
        Roster rosters = allReady.with(p1).with(p2).with(p3).with(Players.make("a"));
