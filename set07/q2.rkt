...
(define (ts-fn ts)
  (cond
    [(empty? ts) ...]
    [(variable? (first ts)) ...(first ts)
                            (ts-fn (rest ts))]
    [else ...(first ts)
          (ts-fn (rest ts))]))

; type-of-exp: ArithmeticExpression VarTypes -> Type
; GIVEN: a ArithmeticExpression exp and VarTypes var-types
; Where: var-types is a list of all variables and their types 
; defined by some larger expression which are available for exp
; variables in exp must be defined
; RETURNS: the type of exp or in var-types
...
; Where: var-types is a list of all variables and their types 
; defined by some larger expression which are available for c
; variables in c must be defined or in var-types
...
; well-typed-exps: ArithmeticExpressionList VarTypes -> Type
; GIVEN: a ArithmeticExpressionList lst and a VarTypes var-types
; Where: lst is the operands of a call 
; and var-types is a list of all variables and their types 
; defined by some larger expression which are available for every exp in lst
; variables in every exp in lst must be defined or in var-types
; Returns: true if every type of exp in lst is Int
; Strategy: use HOF map on lst followed by andmap
; EXAMPLES: (well-typed-exps? (list (lit 1)) empty) => true
(define (well-typed-exps? lst var-types)
  (andmap (lambda (t) (equal? "Int" t)) 
          (map (lambda (x) (type-of-exp x var-types)) lst)))
; Test
(begin-for-test
...
; variables in b must be defined or in var-types
...
; GIVEN: a Block b and a VarTypes var-types
; Where: variables in b must be defined and 
; var-types must contain vars which are not defined by b
...
(define (types-update b var-types)
  (let ([var (block-var b)]
    ...
    ; the the type of the var defined by b 
